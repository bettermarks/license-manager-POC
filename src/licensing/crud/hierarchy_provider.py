...
from licensing.client.hierarchy_provider_client import (
    hierarchy_provider_memberships_url,
)
...
async def get_hierarchy_provider(
    ...
    return (
        await session.execute(
            statement=select(model.HierarchyProvider).where(
...
async def get_user_memberships(
    ...
    try:
        memberships_raw = await http_get(
            hierarchy_provider_memberships_url(f"{url}/", user_eid)
        )
