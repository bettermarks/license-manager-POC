...
export class LicensingServiceAccount extends Chart {
  ...
  constructor(scope: Construct, id: string, props: LicensingServiceAccountProps) {
    super(scope, id);

    const { name, namespace = Namespace.LICENSING } = props;

    this.serviceAccount = new KubeServiceAccount(this, "licensing-service-account", {
      metadata: {
        name,
        namespace,
      },
      imagePullSecrets: props.imagePullSecrets?.map((secretRef) => ({ name: secretRef })),
    });
    ...
    new KubeRoleBinding(this, "licensing-role-binding", {
      ...
      subjects: [{ kind: this.serviceAccount.kind, name: this.serviceAccount.name }],
    });
  }
}
